...
namespace accio {
  ...
  inline buffer<class charT, class copy, class alloc>::buffer(size_type size) {
  ...
  template <class charT, class copy, class alloc>
  inline buffer<class charT, class copy, class alloc>::buffer(char_type *bytes, size_type size, bool cpy) {
    if(nullptr == bytes) {
  ...
  template <class charT, class copy, class alloc>
  inline buffer<class charT, class copy, class alloc>::~buffer() {
    if(nullptr != m_buffer) {
      delete [] m_buffer;
    }
    m_buffer = nullptr;
  ...
  template <class charT, class copy, class alloc>
  inline size_type buffer<class charT, class copy, class alloc>::bufcpy(char_type *data, size_type size) {
    if(nullptr == data) {
      setstate(std::ios_base::badbit);
      return 0;
    }
  ...
  template <class charT, class copy, class alloc>
  inline pos_type buffer<class charT, class copy, class alloc>::seekoff(off_type off, seek_dir way) {
    // from beginning
  ...
  template <class charT, class copy, class alloc>
  inline size_type buffer<class charT, class copy, class alloc>::expand(size_type len) {
    if(0 == len) {
      return 0;
    }
    allocator_type allocator;
  ...
  template <class charT, class copy, class alloc>
  inline size_type buffer<class charT, class copy, class alloc>::read(char_type *data, size_type memlen, size_type count) {
  ...
  template <class charT, class copy, class alloc>
  inline size_type buffer<class charT, class copy, class alloc>::write(const char_type *data, size_type memlen, size_type count) {
    if((nullptr == data) or (0 == memlen) or (0 == count)) {
      return 0;
    }
    if(nullptr == m_buffer) {
